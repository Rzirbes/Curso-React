...
export default function () {
    ...
    return (
        ...
        >
            {!mostrar ? (
                <Botao texto="Abrir" cor="bg-cyan-500" onClick={toggleMostrar}></Botao>
            ) : (
                ""
            )}
            {mostrar ? (
                <Flex center col className={`
                    w-80 h-80 bg-zinc-500 text-white
                    text-2xl p-4 m-4 rounded-md
                `}>
                    <Botao cor="bg-red-700" tamanho="xl" icone={<IconX />} onClick={toggleMostrar}></Botao>
